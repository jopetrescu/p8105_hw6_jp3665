...
Perform simple linear regression with `tmax` as the response and `tmin` as the predictor. Extract r^2 value and log(intercept * slope)
...
weather_bootstrap <- weather_bootstrap %>%
  unnest(glance) %>%
...
weather_results %>% ggplot(aes(x = stat, y = value)) +
  geom_violin() +
